...
func init() {
	...
	flag.BoolVar(&parseQueryLog, "p", false, "parse anduard home log file mode, default false")
	flag.StringVar(&queryLog, "l", "", "adguard home query log file")
	flag.Parse()
}
...
func lookup2(domain string) (ip string, err error)  {
...
func IpCode(ip string) string {
	response, err := http.Get(config.IpInfoServer + "?" + ip)
	if err != nil {
		log.Printf(err.Error())
		return ""
	}
...
func searchDomain(domain string) string  {
	return domainRegex.FindString(domain)
}
func main() {
